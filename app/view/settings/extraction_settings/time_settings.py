...
class class_break_settings(GroupHeaderCardWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setTitle(get_content_name_async("time_settings", "class_break_settings", "name"))
        self.setBorderRadius(8)

        # 课间禁用开关
        self.class_break_switch = SwitchButton()
        self.class_break_switch.setOffText(get_content_name_async("time_settings", "disable"))
        self.class_break_switch.setOnText(get_content_name_async("time_settings", "enable"))
        ...
        self.addGroup(
            ...
            get_content_name_async("time_settings", "class_break_function", "description"),
...
class cses_import_settings(GroupHeaderCardWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setTitle(get_content_name_async("time_settings", "cses_import_settings", "name"))
        self.setBorderRadius(8)

        # 导入文件按钮
        self.import_file_button = PushButton(get_content_name_async("time_settings", "import_from_file"))
        self.import_file_button.setIcon(get_theme_icon("ic_fluent_folder_open_20_filled"))
        self.import_file_button.clicked.connect(self.on_import_file_clicked)

        # 查看模板按钮
        self.view_template_button = PushButton(get_content_name_async("time_settings", "view_template"))
        self.view_template_button.setIcon(get_theme_icon("ic_fluent_document_20_filled"))
        self.view_template_button.clicked.connect(self.on_view_template_clicked)

        # 当前课程表信息标签
        self.schedule_info_label = QLabel(get_content_name_async("time_settings", "no_schedule_imported"))
    ...
    def _update_schedule_info(self):
        ...
        try:
            settings_path = get_settings_path()
            if not file_exists(settings_path):
                self.schedule_info_label.setText(get_content_name_async("time_settings", "no_schedule_imported"))
                return
            ...
            if non_class_times:
                ...
                self.schedule_info_label.setText(
                    get_content_name_async("time_settings", "schedule_imported").format(count)
                )
            else:
                self.schedule_info_label.setText(get_content_name_async("time_settings", "no_schedule_imported"))
    ...
    def _import_cses_file(self, file_path: str):
        ...
        try:
            ...
            self.import_file_button.setText(get_content_name_async("time_settings", "importing"))
            ...
            InfoBar.error(
                title=get_content_name_async("time_settings", "import_failed"),
                content=get_content_name_async("time_settings", "import_error").format(str(e)),
                orient=Qt.Horizontal,
                isClosable=True,
                position=InfoBarPosition.TOP,
                duration=5000,
            ...
            self.import_file_button.setText(get_content_name_async("time_settings", "import_from_file"))
